...
<body onload="registre_selectionne('<?php echo isRegistre() ?>')">
	...
	<fieldset>
		<p>
		<form id="form0" action="registre_mariage.php" method="post">
		<input id="bouton" type="button" value="Nouvel enregistrement" onclick="document.location='form_mariage.php'">
		<label >Registre <select onchange="recharger_page('registre_mariage.php')" value="<?php isRegistre() ?>" name="registre"> <option></option><?php getRegistre($database,"SELECT codeReg FROM registre WHERE nomReg='Mariage'"); ?> </select> </label>
		...
		<div id="registre">
			...
			if (isset($_POST["elt"])) {
				$query="SELECT * FROM acteMariage WHERE codeReg='".$_POST["registre"]."' AND (nomHommeComplet LIKE '%".$_POST["elt"]."%' OR nomFemmeComplet LIKE '%".$_POST["elt"]."%')";			
			}
			else{
				$query="SELECT * FROM acteMariage WHERE codeReg='".isRegistre()."'  ORDER BY numActeMariage ASC";;
			}
			$model->setModelMariage($query); 
			?>
		</div>
	</section>
	<footer> </footer>
